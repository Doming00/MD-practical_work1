...
header-includes:
- \usepackage{titling}
- \pretitle{\begin{center}\LARGE\includegraphics[width=12cm]{first.jpg}\\[\bigskipamount]}
...
barplot(tapply(dades[[9]], P, mean),main=paste("Means of", names(dades)[9], "by", nameP ))
abline(h=mean(dades[[9]]))
legend(0,mean(dades[[9]]),"global mean",bty="n")
```
...
#Test Chi quadrat:
chisq.test(dades[,2], as.factor(P))
```
...
#Test Chi quadrat:
chisq.test(dades[,4], as.factor(P))
```
...
#Test Chi quadrat:
chisq.test(dades[,6], as.factor(P))
```
...
#Test Chi quadrat:
chisq.test(dades[,18], as.factor(P))
```
...
#Test Chi quadrat:
chisq.test(dades[,19], as.factor(P))
```
...
#Test Chi quadrat:
chisq.test(dades[,20], as.factor(P))
```
...
#Test Chi quadrat:
chisq.test(dades[,5], as.factor(P))
```

The first categorical variable we are going to profile is the front camera quality. We can see in the contingency table and the proportion by groups that all the groups are not distributed similarly, but it doesn't seems to be enough differences between them to explain each groups by the variable categories.
The barplots shows us the same, so we will check the Chi-Squared test to find if the variable and the clusters are independent. 
As we see, the p-value is not significant enough to consider the variable and the groups dependant, so we can conclude saying that the front camera quality is not a variable that explains the different clusters.

```{r}
paste("Variable", names(dades)[11])
#Cross-table
...
#Test Chi quadrat:
chisq.test(dades[,11], as.factor(P))
```

We can find a similar behaviour with the variable main camera quality. The groups have a distribution that variates a little for each group, but again, it doesn't seems to be different enough to consider the main camera a variable that explains the clusters. The Chi-Squared test confirms us that, even though the p-value is lower than in the first categorical variable.

```{r}
paste("Variable", names(dades)[14])
#Cross-table
...
#Test Chi quadrat:
chisq.test(dades[,14], as.factor(P))

#relevant variable
#valorsTest:
ValorTestXquali(P,dades[,14])

marg <- table(as.factor(P))/n
#condicionades a classes amb llegenda
...
With this variable we can start to find some differences between clusters. Having a lot of megabytes of RAM seems to be a key factor to represent the fourth group, and having a small and middle RAM seems to explain the second cluster. The barplots doesn't give us a lot of information so we will check with the Chi-Squared test if the variable and the clusters are dependant. The p-value that the test gives us is significant enough to consider the RAM categorical variable relevant, therefore we will do the test-values to see the relation between proportions of the category in every cluster and the proportion of all the data, and the p-values of each category and group.

We can see that the first group is related with the Low and Middle RAM category. The second group is related with all three categories, especially with the Low Ram, since it has a very low p-value. The third cluster is not related with any RAM category, but the fourth one is also strongly related with the High Ram category. We can visually see this doing a snake plot.

```{r}
paste("Variable", names(dades)[22])
#Cross-table
...
#Test Chi quadrat:
chisq.test(dades[,22], as.factor(P))
```

The next variable we will profile is the screen dimension variable. Remember that this variables is a factor variable from two numerical variables that represented the screen height and width. With the contingency table and the proportions by row we check that the first group contains a bigger quantity of small phones, the third group has less medium sized phones and the last group has the most quantity of big phones. Even though the distribution is not the same for all the clusters, it doesn't variate enough to consider this variables as an explanatory, which the Chi-Square test confirms giving us a not enough significant p-value.

```{r}
paste("Variable", names(dades)[23])
#Cross-table
...
#Test Chi quadrat:
chisq.test(dades[,23], as.factor(P))
```

The last variable we profile is the Pixels per Inch (pixel ratio) variable. We can see that the categories are equally distributed between the clusters, although the first and last groups seem to differ a bit with all the others. Doing the Chi-Squared test we can find if this differences are enough to consider this variable related to the clusters. The p-value gives us a high number, so it's not sufficiently significant to consider a relations between the variable and the groups.

## Classes' interpretation

After profiling every category with the clusters we found doing the hierarchical clustering, we can try to interpret the cluster classes with the information we extracted in the previous sections. 

The first cluster is characterized by having a very low battery power, much less than the average and the other classes, and contains mobiles without a High Ram capacity. The second cluster has mobiles with very low RAM and more battery power than the first group, but still bellow the average. The third cluster is the one that contains mobiles with very high battery power. Finally, the fourth group is characterized by mobiles with high ram capacity and with the less weight in comparison with the other groups.
...
## Porjection of illustrative qualitative variables on individuals' map
