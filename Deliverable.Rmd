...
library(readr)
df <- read_csv("group deliverable/data.csv")
summary(df)
```
...
#knn imputation of sc_w NaN values using all the other numerical categories
dfaux<-df[,c(var_num,"fc","px_height", "px_width","ram","sc_h","price_range")]
names(dfaux)
aux1 <- subset(dfaux, !is.na(df$sc_w) | !is.na(df$pc))#dfaux[subs(!is.na(df$sc_w),!is.na(df$pc)),]
aux2 <- dfaux[is.na(c(df$sc_w,df$pc)),]

knn.ing = knn(aux1,aux2,df$sc_w[!is.na(c(df$sc_w,df$pc))])
